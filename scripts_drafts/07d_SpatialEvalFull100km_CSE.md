Spatial Evaluation /\< 100 km - Full RF model of raw CSE
================
Norah Saarman
2026-04-01
...
- `../input/Gff_cse_envCostPaths.csv` - Combined CSE table with
  ...
  model.
- `../results_dir/fullRF100km_CSE.tif` - Final full model projected
  estimates of CSE at 1 pixel geodist, max 100 km geodist.
...
`spatial_eval_rawCSE100km_LCPpred.csv` Pairwise least-cost path
summaries for each site pair: LCP_mean, LCP_sum. -
`spatial_eval_rawCSE100km_cber.csv` Pairwise circuit-based effective
resistance (CBER) values for each site pair. -
`spatial_eval_rawCSE100km_predictions.csv` Combined pairwise evaluation
table containing observed CSE and spatial predictions for each site
pair: CSE, LCP_mean, LCP_sum, CBER. -
`spatial_eval_rawCSE100km_eval_metrics.csv` Summary table of evaluation
...
`least_cost_paths/LC_paths_fullRF_rawCSE100km.shp` Saved least-cost-path
...
changes. - `spatial_eval_rawCSE100km_LCPpred.csv` Backup copy of
least-cost path summaries. - `spatial_eval_rawCSE100km_cber.csv` Backup
copy of circuit-based effective resistance (CBER) results. -
`spatial_eval_rawCSE100km_predictions.rds` R object version of the
combined evaluation table (eval_results) for quick reloading in later
scripts. - `spatial_eval_rawCSE100km_predictions.csv` Backup copy of the
combined evaluation table saved in the large-results directory. -
`spatial_eval_rawCSE100km_predictions_calibrated.rds` R object version
of the evaluation table with calibrated predictions (eval_df), including
LCP_mean_cal, LCP_sum_cal, and CBER_cal. -
`spatial_eval_rawCSE100km_eval_metrics.rds` R object version of the
evaluation metrics table (metrics_df) for quick reloading in later
scripts. - `spatial_eval_rawCSE100km_eval_metrics.csv` Backup copy of
the evaluation metrics table saved in the large-results directory.
...
estimates of raw CSE at 1 pixel geodist, max 100 km) to estimate
evaluation metrics by 3 methods:  
...
surf_file  <- file.path(results_dir_big, "fullRF100km_CSE.tif")
########################################
...
```
    ...
    ##  0.1809  0.3038  0.3275  0.3221  0.3491  0.4121
...
```
    ...
    ##    Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
    ##  0.1816  0.3159  0.3353  0.3378  0.3588  0.4121
...
![](07d_SpatialEvalFull100km_CSE_files/figure-gfm/check-alignment-1.png)<!-- -->
...
![](07d_SpatialEvalFull100km_CSE_files/figure-gfm/sanity-1.png)<!-- -->
...
![](07d_SpatialEvalFull100km_CSE_files/figure-gfm/test-lcp-1.png)<!-- -->
Run full loop:
...
# save paths
st_write(paths_sf,
         file.path(output_dir, "LC_paths_fullRF_rawCSE100km.shp"),
         delete_layer = TRUE)
```

    ## Writing layer `LC_paths_fullRF_rawCSE100km' to data source 
    ##   `/uufs/chpc.utah.edu/common/home/saarman-group1/uganda-tsetse-LG/results//least_cost_paths/LC_paths_fullRF_rawCSE100km.shp' using driver `ESRI Shapefile'
...
paths_sf <- st_read(file.path(results_dir_big, "least_cost_paths", "LC_paths_fullRF_rawCSE100km.shp"), quiet = TRUE)
...
```
    ...
    ##    Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
    ##  0.2444  0.2782  0.2909  0.2989  0.3103  0.4010
...
```
    ...
    ##    Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
    ##   1.649  33.842  60.749  67.181  93.176 216.762

``` r
# Save LCP output as text file in results directories
write.csv(LCP_results,file.path(results_dir, "spatial_eval_rawCSE100km_LCPpred.csv"), row.names = FALSE)

write.csv(LCP_results,file.path(results_dir_big, "spatial_eval_rawCSE100km_LCPpred.csv"), row.names = FALSE)
```
...
```
    ...
    ##     Var1   Var2            id    CBER
    ## 1 01-AIN 02-GAN 01-AIN_02-GAN 1570776
    ## 2 01-AIN 03-DUK 01-AIN_03-DUK 1493046
    ## 3 02-GAN 03-DUK 02-GAN_03-DUK 1374770
    ## 4 03-DUK 07-OSG 03-DUK_07-OSG 2207692
    ## 5 02-GAN 07-OSG 02-GAN_07-OSG 2207597
    ## 6 01-AIN 07-OSG 01-AIN_07-OSG 2198607
...
```
    ...
    ##  $ CBER: num  1570776 1493046 1374770 2207692 2207597 ...

``` r
# save output in results directories
write.csv(cber_results,file.path(results_dir, "spatial_eval_rawCSE100km_cber.csv"), row.names = FALSE)

write.csv(cber_results,file.path(results_dir_big, "spatial_eval_rawCSE100km_cber.csv"), row.names = FALSE)
```
...
LCP_results <- read.csv(file.path(results_dir, "spatial_eval_rawCSE100km_LCPpred.csv"))

# load the CBER results you just computed
cber_results <- read.csv(file.path(results_dir, "spatial_eval_rawCSE100km_cber.csv"))
...
saveRDS(
  eval_results,
  file.path(results_dir_big, "spatial_eval_rawCSE100km_predictions.rds")
)
```
...
![](07d_SpatialEvalFull100km_CSE_files/figure-gfm/calibrate-and-metrics-1.png)<!-- -->
...
![](07d_SpatialEvalFull100km_CSE_files/figure-gfm/calibrate-and-metrics-2.png)<!-- -->
...
![](07d_SpatialEvalFull100km_CSE_files/figure-gfm/calibrate-and-metrics-3.png)<!-- -->
...
```
    ...
    ## 1 LCP_mean 1091 0.007272799 0.08528071 0.06755364 0.1031797   0.3212160
    ## 2  LCP_sum 1091 0.003152370 0.05614597 0.04563823 0.6112763   0.7818416
    ## 3     CBER 1091 0.003275343 0.05723061 0.04419454 0.5961124   0.7720832

``` r
# Save outputs
saveRDS(
  metrics_df,
  file.path(results_dir_big, "spatial_eval_rawCSE100km_eval_metrics.rds")
)

write.csv(
  metrics_df,
  file.path(results_dir_big, "spatial_eval_rawCSE100km_eval_metrics.csv"),
  row.names = FALSE
)

write.csv(
  metrics_df,
  file.path(results_dir, "spatial_eval_rawCSE100km_eval_metrics.csv"),
  row.names = FALSE
)
```
