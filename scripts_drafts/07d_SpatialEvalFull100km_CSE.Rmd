---
title: "Spatial Evaluation /< 100 km - Full RF model of raw CSE"
...
## Inputs
  ...
  - `../results_dir/fullRF100km_CSE.tif` - Final full model projected estimates of CSE at 1 pixel geodist, max 100 km geodist.
...
**Small text outputs saved to results_dir within git repo:**
 - `spatial_eval_rawCSE100km_LCPpred.csv` Pairwise least-cost path summaries for each site pair: LCP_mean, LCP_sum.
 - `spatial_eval_rawCSE100km_cber.csv` Pairwise circuit-based effective resistance (CBER) values for each site pair.
 - `spatial_eval_rawCSE100km_predictions.csv` Combined pairwise evaluation table containing observed CSE and spatial predictions for each site pair: CSE, LCP_mean, LCP_sum, CBER.
 - `spatial_eval_rawCSE100km_eval_metrics.csv` Summary table of evaluation metrics for each spatial summary method (LCP_mean, LCP_sum, CBER): RMSE, R², MSE, MAE, Correlation.

**Large or reusable outputs saved to results_dir_big:**
 - `least_cost_paths/LC_paths_fullRF_rawCSE100km.shp` Saved least-cost-path spatial lines for all evaluated site pairs. This is a reusable spatial output and does not need to be recomputed unless the resistance surface changes.
 - `spatial_eval_rawCSE100km_LCPpred.csv` Backup copy of least-cost path summaries.
 - `spatial_eval_rawCSE100km_cber.csv` Backup copy of circuit-based effective resistance (CBER) results.
 - `spatial_eval_rawCSE100km_predictions.rds` R object version of the combined evaluation table (eval_results) for quick reloading in later scripts.
 - `spatial_eval_rawCSE100km_predictions.csv` Backup copy of the combined evaluation table saved in the large-results directory.
 - `spatial_eval_rawCSE100km_predictions_calibrated.rds` R object version of the evaluation table with calibrated predictions (eval_df), including LCP_mean_cal, LCP_sum_cal, and CBER_cal.
 - `spatial_eval_rawCSE100km_eval_metrics.rds` R object version of the evaluation metrics table (metrics_df) for quick reloading in later scripts.
 - `spatial_eval_rawCSE100km_eval_metrics.csv` Backup copy of the evaluation metrics table saved in the large-results directory.
...
**For this script (7d):**  
Run spatial evaluation across the raw CSE resistance surface (projected estimates of raw CSE at 1 pixel geodist, max 100 km) to estimate evaluation metrics by 3 methods:   
