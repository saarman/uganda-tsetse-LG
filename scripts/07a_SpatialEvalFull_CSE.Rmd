...
NOTE: I tried to run in parallel as well, but there were errors that I couldn't solve.
```{r cyber, eval = TRUE}
# Compute circuit-based effective resistance (CBER) sequentially
cber_list <- vector("list", nrow(site_pairs))

pb <- txtProgressBar(min = 0, max = nrow(site_pairs), style = 3)

for (k in seq_len(nrow(site_pairs))) {
  ...
  cber_list[[k]] <- data.frame(
    Var1 = site_pairs$Var1[k],
    Var2 = site_pairs$Var2[k],
    id   = site_pairs$id[k],
    CSE  = site_pairs$CSE[k],
    CBER = dval
  )
...
cber_df <- do.call(rbind, cber_list)
...
# helper: fit linear calibration and return calibrated predictions
...
# optional: plots of calibrated predictions vs observed
plot1_cal <- ggplot(eval_df, aes(x = LCP_sum_cal, y = CSE)) +
  geom_point() +
  geom_smooth(method = "lm") +
  labs(
    x = "Predicted CSE (LCP sum, calibrated)",
    y = "Observed CSE"
  )
plot1_cal

plot2_cal <- ggplot(eval_df, aes(x = LCP_mean_cal, y = CSE)) +
  geom_point() +
  geom_smooth(method = "lm") +
  labs(
    x = "Predicted CSE (LCP mean, calibrated)",
    y = "Observed CSE"
  )
plot2_cal

plot3_cal <- ggplot(eval_df, aes(x = CBER_cal, y = CSE)) +
  geom_point() +
  geom_smooth(method = "lm") +
  labs(
    x = "Predicted CSE (CBER, calibrated)",
    y = "Observed CSE"
  )
#plot3_cal

# calculate formal metrics
# correlation is kept on raw predictions because it is scale-invariant
