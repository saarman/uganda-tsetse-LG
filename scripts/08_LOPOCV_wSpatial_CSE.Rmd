...
```{r 5-viz-spearman, warning = FALSE}
...
### %IncMSE
```{r 7-variable-imp-mse-plus-pca-pruned}
# Load full model
...
```
### NodePurity
```{r 7-variable-imp-nodepurity-plus-pca-pruned}
# Load full model
full_model <- readRDS(file.path(results_dir, "rf_mean_full_tuned.rds"))
...
Helpers
```{r 8a-spatial-helpers, eval = FALSE}
calibrate_pred <- function(obs_train, pred_train, pred_test) {
  keep <- complete.cases(obs_train, pred_train)
  df <- data.frame(obs = obs_train[keep], pred = pred_train[keep])
  fit <- lm(obs ~ pred, data = df)
