...
```{r moran-non-spatial, eval = FALSE, include=FALSE}
# -----------------------------------
# NEW: Moran's I on site-level LOPOCV error
# -----------------------------------

# Metrics for fold-level moran's on rmse and mae:
library(spdep)

# build one coordinate per site
site_coords <- bind_rows(
  V.table %>% dplyr::select(site = Var1, long = long1, lat = lat1),
  V.table %>% dplyr::select(site = Var2, long = long2, lat = lat2)
) %>%
  distinct()

# join coordinates to fold-level site summaries
moran_df <- metrics_all %>%
  left_join(site_coords, by = "site") %>%
  filter(complete.cases(long, lat, RMSE, MAE))

coords <- as.matrix(moran_df[, c("long", "lat")])

# k-nearest-neighbor graph
nb <- knn2nb(knearneigh(coords, k = 6))
lw <- nb2listw(nb, style = "W")

# test spatial clustering of site-level prediction error
moran_rmse <- moran.test(moran_df$RMSE, lw)
moran_mae  <- moran.test(moran_df$MAE, lw)

print(moran_rmse)
print(moran_mae)

moran_summary_v2 <- data.frame(
  metric = c("RMSE", "MAE"),
  Moran_I = c(unname(moran_rmse$estimate[1]), unname(moran_mae$estimate[1])),
  expected = c(unname(moran_rmse$estimate[2]), unname(moran_mae$estimate[2])),
  variance = c(unname(moran_rmse$estimate[3]), unname(moran_mae$estimate[3])),
  p_value = c(moran_rmse$p.value, moran_mae$p.value)
)

write.csv(
  moran_summary_v2,
  file.path(results_dir, "LOPOCV_site_error_moran_v2.csv"),
  row.names = FALSE
)

write.csv(
  moran_df,
  file.path(results_dir, "LOPOCV_site_error_table_v2.csv"),
  row.names = FALSE
)
```
...
```{r 9-fold-spatial-metrics, eval = FALSE}
## Chunk 9: Evaluative metrics
...
```{r spatial-morans, eval = FALSE, include=FALSE}
# -----------------------------------
# NEW: Moran's I on site-level spatial LOPOCV error
# -----------------------------------
library(spdep)
...
write.csv(
  moran_summary,
  file.path(results_dir, "spatial_LOPOCV_LCPsum_k1_moran_v2.csv"),
  row.names = FALSE
)

# optional save of joined site table
write.csv(
  moran_df,
  file.path(results_dir, "spatial_LOPOCV_LCPsum_k1_site_error_v2.csv"),
  row.names = FALSE
)
