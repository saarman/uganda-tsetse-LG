...
plot(sdm,
     ...
     legend.args = list(text = "Suitability", side = 2, line = 2.5, cex = 0.8))
plot(st_geometry(lakes), col = "black", border = NA, add = TRUE)
plot(st_geometry(uganda_cropped), border = "black", lwd = .25, add = TRUE)
```
...
## 9a. Connectivity and Suitability bivmap companion maps with custom colors
...
```{r plot-raw-env}
# Convert env to SpatRaster for terra functions
raw_stack <- terra::rast(env)

for (nm in names(raw_stack)) {
  r <- raw_stack[[nm]]
  title <- nm

  # Crop/mask to reference
  r <- terra::crop(r, ref_raster)
  r <- terra::mask(r, ref_raster)

  # Range and mean
  zr   <- terra::global(r, fun = range, na.rm = TRUE, names = FALSE)
  zmin <- zr[1, 1]
  zmax <- zr[1, 2]
  zmu  <- as.numeric(terra::global(r, fun = "mean", na.rm = TRUE))

  n <- 100

  if (is.na(zmin) || is.na(zmax) || zmin == zmax) {
    cols <- rep("white", n)
    zlim <- c(0, 0)
  } else {
    below <- max(0, zmu - zmin)
    above <- max(0, zmax - zmu)
    total <- below + above
    if (total == 0) {
      zmu <- (zmin + zmax) / 2
      below <- zmu - zmin
    ...
    n_neg <- max(1, round(n * below / total))
    n_pos <- max(1, n - n_neg)

    # blue (below mean) → white → warm (above mean)
    neg_col <- colorRampPalette(c("blue4", "deepskyblue3", "#8db4c9", "white"))(n_neg)
    pos_col <- colorRampPalette(c("white", "gold", "darkorange", "#924532"))(n_pos)
  ...
  plot(r,
       ...
       main = title,
       axes = FALSE, box = FALSE,
       legend.args = list(text = "Value", side = 1, line = 2.5, cex = 0.8))

  # Overlays (assumes lakes_v and uganda_v already created)
  plot(lakes_v,  col = "black", border = NA, add = TRUE)
  plot(uganda_v, border = "black", lwd = 0.25, add = TRUE)
}
