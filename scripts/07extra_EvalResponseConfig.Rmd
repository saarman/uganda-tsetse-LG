---
title: "Spatial Evaluation of Alternative Response Formulations"
date: "`r Sys.Date()`"
output: pdf_document
---
...
Instead, it evaluates alternative formulations of genetic distance and alternative approaches for summarizing predicted resistance across the landscape. This is a screening step using the full model only, with the goal of selecting the response formulation and spatial summary to carry forward into the final LOPOCV validation framework.
...
knitr::opts_chunk$set(
  echo = TRUE,
  # Knit figures are written only to a temporary session directory
  # so existing figure files are never overwritten.
  fig.path = file.path(tempdir(), "knitted_mds/")
)
```
