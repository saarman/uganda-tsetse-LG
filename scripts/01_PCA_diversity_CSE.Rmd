...
# compute great-circle geographic distance (km)
cse_combined <- cse_combined %>%
  mutate(
    geo_km = distHaversine(
      cbind(long1, lat1),
...
# prepare clusters and results list
clusters <- c("Northern", "Southern")
mantel_results <- list()

for (cl in clusters) {
  df <- cse_combined %>% filter(Pop1_cluster == cl, Pop2_cluster == cl)
  pops <- sort(unique(c(df$Var1, df$Var2)))
  n    <- length(pops)

  # initialize empty distance matrices
  mat_gen <- matrix(0, n, n, dimnames = list(pops, pops))
  mat_geo <- mat_gen

  # fill matrices with pairwise values
  for (i in seq_len(nrow(df))) {
    v1 <- df$Var1[i]; v2 <- df$Var2[i]
    mat_gen[v1, v2] <- df$CSEdistance[i]
    mat_gen[v2, v1] <- df$CSEdistance[i]
    mat_geo[v1, v2] <- df$geo_km[i]
    mat_geo[v2, v1] <- df$geo_km[i]
  }

  # convert to dist objects
  gen_dist <- as.dist(mat_gen)
  geo_dist <- as.dist(mat_geo)

  # run Mantel test with 999 permutations
  mt <- mantel.rtest(gen_dist, geo_dist, nrepet = 999)
  mantel_results[[cl]] <- mt
}

# view Mantel correlation and p-value for each cluster
mantel_results$Northern
mantel_results$Southern

# Display plots of IBD
#pdf("../figures/ibd_mantel.pdf", width = 14, height = 10)
ibd_figure
#dev.off()  
```