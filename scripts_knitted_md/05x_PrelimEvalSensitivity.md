Eval. Response Formulations & River Kernel Sensitivity
================
Norah Saarman
2026-08-14

- [Overview of script](#overview-of-script)
- [1. Evalute alternative response
  formuations](#1-evalute-alternative-response-formuations)
  ...
  - [Raw CSE](#raw-cse)
    ...
    - [Step 7: Extract predicted CSE values along LCPs and calculate
      mean and
  ...
  - [CSE per km (as a rate)](#cse-per-km-as-a-rate)
...
1. Evaluate alternative response formulations (raw CSE, IBD residuals,
CSE/km). 2. Evaluate alternative river density bandwidths (1, 2, 3, 5,
10 km).

# 1. Evalute alternative response formuations

This is a screening step using the full model only, with the goal of
selecting the response formulation and spatial summary to carry forward
into the final LOPOCV validation framework.
...
![](../figures/scripts_knitted_mds/raw-check-alignment-1.png)<!-- -->
...
![](../figures/scripts_knitted_mds/resid-check-alignment-1.png)<!-- -->
...
![](../figures/scripts_knitted_mds/rate-check-alignment-1.png)<!-- -->
...
![](../figures/scripts_knitted_mds/results-rsq-plot-1.png)<!-- -->
...
![](../figures/scripts_knitted_mds/results-correlation-plot-1.png)<!-- -->
...
![](../figures/scripts_knitted_mds/results-prediction-plots-1.png)<!-- -->

    ## `geom_smooth()` using formula = 'y ~ x'

![](../figures/scripts_knitted_mds/results-prediction-plots-2.png)<!-- -->

    ## `geom_smooth()` using formula = 'y ~ x'

![](../figures/scripts_knitted_mds/results-prediction-plots-3.png)<!-- -->

    ## `geom_smooth()` using formula = 'y ~ x'

![](../figures/scripts_knitted_mds/results-prediction-plots-4.png)<!-- -->

    ## `geom_smooth()` using formula = 'y ~ x'

![](../figures/scripts_knitted_mds/results-prediction-plots-5.png)<!-- -->

    ## `geom_smooth()` using formula = 'y ~ x'

![](../figures/scripts_knitted_mds/results-prediction-plots-6.png)<!-- -->

    ## `geom_smooth()` using formula = 'y ~ x'

![](../figures/scripts_knitted_mds/results-prediction-plots-7.png)<!-- -->

    ## `geom_smooth()` using formula = 'y ~ x'

![](../figures/scripts_knitted_mds/results-prediction-plots-8.png)<!-- -->

    ## `geom_smooth()` using formula = 'y ~ x'

![](../figures/scripts_knitted_mds/results-prediction-plots-9.png)<!-- -->
...
# River kernel bandwidths to test
river_km <- c(1, 2, 3, 5, 10)

# Load modeling table and apply final-model filtering
V.table <- read.csv(
...
# Predictors used in final model, excluding river density
base_predictors <- c(
...
# Final predictor names used by random forest
predictor_names <- c(
...
# Load existing least-cost paths
lines_sf <- st_read(
...
river_models <- setNames(
  vector("list", length(river_km)),
  paste0("river_", river_km, "km")
)
...
for (j in seq_along(river_km)) {
  ...
  # Mean river density along each least-cost path
  river_mean <- raster::extract(
    river_raster,
    lines_sp
  ) %>%
    vapply(
  ...
  # Match predictor names used in final model
  names(model_df) <- gsub("_mean$", "", names(model_df))
  model_df <- model_df[, c("CSEdistance", predictor_names)]

  # Fit random forest using final-model settings
  set.seed(92834567)

  rf_k <- randomForest(
    x = model_df[, predictor_names],
  ...
  # Store performance and river importance
  imp <- importance(rf_k)

  river_sensitivity$OOB_MSE[j] <- tail(rf_k$mse, 1)
  river_sensitivity$VarExplained[j] <- tail(rf_k$rsq, 1) * 100
  river_sensitivity$River_IncMSE[j] <- imp["riv", "%IncMSE"]
  river_sensitivity$River_IncNodePurity[j] <- imp["riv", "IncNodePurity"]
}
...
```
    ...
    ##   bandwidth_km     OOB_MSE VarExplained River_IncMSE River_IncNodePurity
    ## 1            1 0.001149792     85.82173     15.72398           0.1067742
    ## 2            2 0.001152466     85.78877     17.71600           0.1185055
