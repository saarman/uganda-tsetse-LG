...
# Double check extent/cropping/CRS is same as current
envBase <- stack(file.path(data_dir, "processed", "env_stack.grd"))
...
# write to file as .tif as backup
writeRaster(envstack,
            filename = file.path(data_dir, "processed", "env_stack_RCP4_5.tif"),
            format = "GTiff",
            overwrite = FALSE)

write.csv(names(envstack),file.path(data_dir, "processed", "env_stack_layers_RCP4_5.csv"))
```
